...
export class PerformanceMonitor {
  ...
  getStats(timeRangeMs: number = 3600000): {
    ...
    const cacheableRequests = recentMetrics.filter(
      (m) => m.cacheHit !== undefined
    ).length;
    const cacheHitRate =
      cacheableRequests > 0 ? (cacheHits / cacheableRequests) * 100 : 0;
    ...
    const requestsByRoute = recentMetrics.reduce(
      (acc, m) => {
  ...
  getRouteStats(
    ...
    const averageResponseTime =
      routeMetrics.reduce((sum, m) => sum + m.duration, 0) / totalRequests;

    const sortedDurations = routeMetrics
      .map((m) => m.duration)
      .sort((a, b) => a - b);
    ...
    const errorRequests = routeMetrics.filter(
      (m) => m.statusCode >= 400
    ).length;
    const errorRate = (errorRequests / totalRequests) * 100;
